...
namespace Kevin.SyntaxTextBox
{
    ...
    public class SyntaxTextBox : System.Windows.Forms.RichTextBox 
	{
        ...
        public string ConfigFile
        {
            set 
            {
                LoadConfig(GetConfigUrl());
                mConfigFile = GetConfigUrl();
                this.OnTextChanged(null);
            }
            get
            {
                return string.IsNullOrEmpty(mConfigFile) ? GetConfigUrl() : mConfigFile;
            }
        }
        ...
        public string GetConfigUrl()
        {
            ...
            //foreach (string s in listapp)
            //{
            //    result = Environment.GetFolderPath(Environment.SpecialFolder.Personal) + string.Format("\\Visual Studio " + s + "\\Addins\\Generator\\csharp.xml");
            ...
            //return result;
            return this.GetType().Assembly.Location.GetFileDirectory() + "\\csharp.xml";
        }
